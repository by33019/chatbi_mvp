...
product_line VARCHAR(50) 产品线: 动力电池-乘用车 / 动力电池-商用车 / 储能系统-电网级 / 储能系统-工商业
category VARCHAR(50) 产品分类: 高能量密度型 / 超快充型 / 混动专用型 / 低温适配型 / 商用车标准型 / 电网级储能型
...
1. 收入口径:
   "销售额"、"收入"均指不含税收入，
   统一使用 sales_orders.net_amount，
   禁止使用 gross_amount。

2. 成本口径:
   "成本"指实际销售成本，
   计算公式为:
   dim_products.material_cost + dim_products.labor_cost。

3. 订单统计范围:
   统计收入、订单量、客单价等指标时，
   必须过滤 order_status = 'completed'，
   排除 cancelled 和 pending 订单。

4. 汇率转换:
   涉及多币种收入汇总时，
   必须通过 order_date 和 currency
   关联 exchange_rates 表，
   使用 rate_to_cny 转换成人民币。
...
6. 费用层级:
   selling_expense 是销售费用总项，
   marketing_expense、logistics_expense、warranty_expense
   属于销售相关费用子项。
   如果直接统计 selling_expense，不要重复叠加子项。

7. 毛利计算:
   毛利 =
   net_amount - (material_cost + labor_cost) * quantity。
"""
...
字段选择:
确认金额字段是 net_amount（不含税）还是 gross_amount（含税）。
除非用户明确要求“含税”，否则收入统一使用 net_amount。

Join 遗漏:
只要查询涉及收入且存在 currency 字段，
在需要统一人民币口径时必须关联 exchange_rates 表进行汇率转换。
...
聚合维度:
GROUP BY 字段必须与 SELECT 中的非聚合字段完全一致。
"""
...
def build_prompt(
    ...
    Args:
        ...
        indicator_knowledge: 指标知识文本块（第 9 课新增）
...
"""
    ...
    system_msg = (
        "你是一个专业的 SQL 生成助手，"
        "擅长根据业务问题生成标准 MySQL 查询语句。"
        "请严格遵守给定的数据库 Schema 和业务规则。"
    )
