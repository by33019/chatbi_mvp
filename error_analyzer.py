...
负责分类 SQL生成错误类型，提供错误根因分析与修复建议
该模块用于系统性地收集、分类和追踪 Text2SQL系统的错误模式
"""
...
class ErrorAnalyzer:
    ...
    SYNTAX_ERROR = "syntax_error"            # SQL语法错误
    ...
    def categorize_error(
        ...
        if error_msg:
            if self._is_syntax_error(error_msg):
                ...
                result["root_cause"] = f"SQL语法错误: {error_msg}"
                result["suggestion"] = (
                    "检查 SQL语句中的关键字拼写、括号匹配和引号使用，"
                    "必要时在 Prompt 中补充 SQL 格式约束"
                )
                return result

        # 基于 SQL内容和问题语义进行启发式分类
        ...
        if detected_types:
            ...
            result["root_cause"] = self._get_root_cause(
                primary_type,
                sql,
                question
            )
            ...
            result["root_cause"] = "无法从 SQL中直接识别错误模式，需人工审查"
            result["suggestion"] = (
                "将该用例加入 Few-shot示例或增强 Schema描述中的相关字段说明"
            )
    ...
    def _get_root_cause(
        ...
        causes = {
            self.FIELD_ERROR:
                "模型未能正确理解业务语义，选择了错误的字段",

            self.JOIN_ERROR:
                "模型遗漏了必要的表关联或使用了错误的关联条件",

            self.TIME_ERROR:
                "时间范围计算不符合业务预期，动态时间边界处理有误",

            self.FILTER_ERROR:
                "遗漏了关键的业务过滤条件，导致统计范围不正确",

            self.AGGREGATION_ERROR:
                "聚合维度与 SELECT字段不匹配，或聚合逻辑有误",

            self.SYNTAX_ERROR:
                "生成的 SQL存在语法问题，无法被数据库解析",
        }
    ...
    def _get_suggestion(
        ...
        suggestions = {
            self.FIELD_ERROR:
                "在 RULES中强化字段语义定义，增加字段选择错误的 Few-shot负例",

            self.JOIN_ERROR:
                "在 ERROR_GUARDS中补充 Join路径检查清单，增加多表查询 Few-shot示例",

            self.TIME_ERROR:
                "增加时间计算相关的 Few-shot示例，明确动态时间边界的 SQL写法",

            self.FILTER_ERROR:
                "在 RULES中明确各类查询的强制过滤条件，增加过滤遗漏的负例",

            self.AGGREGATION_ERROR:
                "增加 GROUP BY正确用法的示例，强调 SELECT非聚合字段必须出现在 GROUP BY中",

            self.SYNTAX_ERROR:
                "检查 Prompt中是否有格式混乱导致模型输出不完整 SQL的情况",
        }
    ...
    def analyze_batch(
        ...
        Args:
            cases: 每个元素为
                   {"question": str, "sql": str, "error_msg": str}
                   的字典列表
...
if __name__ == "__main__":
    ...
    result = analyzer.categorize_error(
        ...
        question="最近三个月的总收入是多少"
    )

    print(result["error_type"])    # field_error
    print(result["suggestion"])    # 在 RULES中强化字段语义定义...